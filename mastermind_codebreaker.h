...
static int numUniqueDefiniteColors;

void reset_code_breaker() {
	prevGuessI = 0;
	numPossibleColors = NUM_WITH_BLANK;
	numDefiniteColors = 0;
	numUniqueDefiniteColors = 0;
	...
	char keys[NUM_GUESSES][CODE_LENGTH])
{
	...
	for (int i = prevGuessI; i < curGuessI; i++) {
		...
		int numUniqueDefiniteColorsHere = 0;
		...
		for (int j = 0; j < CODE_LENGTH; j++) {
			...
			if (colorPossibility[guesses[i][j]] >= COLOR_IS_DEFINITE) {
				numDefiniteColorsHere++;
				if (cntColors[guesses[i][j]] == 1) {
					numUniqueDefiniteColorsHere++;
				}
			}
		}
		int numNewCorrectColors = numCorrectColors - numDefiniteColorsHere;
		int numNewUniqueColors = numUniqueColors - numUniqueDefiniteColorsHere;
		
		if (numNewCorrectColors == 0) {
			for (int j = 0; j < CODE_LENGTH; j++) {
				if (colorPossibility[guesses[i][j]] == COLOR_IS_POSSIBLE) {
		...
		for (int j = 0; j < NUM_WITH_BLANK; j++) {
			if (colorPossibility[j] == COLOR_IS_POSSIBLE) {
				numPossibleColors++;
			}
			else if (colorPossibility[j] >= COLOR_IS_DEFINITE) {
				numDefiniteColors += colorPossibility[j];
				numUniqueDefiniteColors++;
			}
		}
		printf("Number of definite colors: %d; Unique: %d\n",
			numDefiniteColors, numUniqueDefiniteColors);
		if (numDefiniteColors == CODE_LENGTH) {
			for (int j = 0; j < NUM_WITH_BLANK; j++) {
				if (colorPossibility[j] == COLOR_IS_POSSIBLE) {
					colorPossibility[j] = COLOR_IS_IMPOSSIBLE;
				}
			}
		}
